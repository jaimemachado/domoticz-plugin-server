...
class device:
    def __init__(self, host, mac, devtype, timeout=10):
        ...
        self.key = bytearray([0x09, 0x76, 0x28, 0x34, 0x3f, 0xe9,
                              0x9e, 0x23, 0x76, 0x5c, 0x15, 0x13, 0xac, 0xcf, 0x8b, 0x02])
        self.iv = bytearray([0x56, 0x2e, 0x17, 0x99, 0x6d, 0x09, 0x3d,
                             0x28, 0xdd, 0xb3, 0xba, 0x69, 0x5a, 0x2e, 0x6f, 0x58])
    ...
    def get_energy(self):
        ...
        if err == 0:
            ...
            if type(payload[0x07]) == int:
                energy = int(
                    hex(payload[0x07] * 256 + payload[0x06])[2:]) + int(hex(payload[0x05])[2:]) / 100.0
            else:
                energy = int(hex(ord(payload[0x07]) * 256 + ord(payload[0x06]))[
    ...
    def check_sensors(self):
        ...
        if err == 0:
            ...
            if type(payload[0x4]) == int:
                ...
                data['humidity'] = (ord(payload[0x6]) *
                                    10 + ord(payload[0x7])) / 10.0
...
class hysen(device):
    ...
    def set_advanced(self, loop_mode, sensor, osv, dif, svh, svl, adj, fre, poweron):
        input_payload = bytearray([0x01, 0x10, 0x00, 0x02, 0x00, 0x05, 0x0a, loop_mode, sensor,
                                   osv, dif, svh, svl, (int(adj * 2) >> 8 & 0xff), (int(adj * 2) & 0xff), fre, poweron])
    ...
    def set_temp(self, temp):
        self.send_request(
            bytearray([0x01, 0x06, 0x00, 0x01, 0x00, int(temp * 2)]))
    ...
    def get_sensors_status(self):
        ...
        if err == 0:
            ...
            if payload:
                ...
                sensors = payload[0x6:]
                sensors_a = [bytearray(sensors[i * 83:(i + 1) * 83])
